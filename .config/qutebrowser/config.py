...
#   - no-unknown-3rdparty: Accept cookies from the same origin only, unless a cookie is already set for the domain. On QtWebEngine, this is the same as no-3rdparty.
#   - never: Don't accept cookies at all.
config.set('content.cookies.accept', 'no-3rdparty', 'https://*')
...
# Key bindings
config.bind(
    'M', 'hint links spawn sh /home/joshii/.scripts/queue_video.sh {hint-url}')
config.bind(
    'N',
    'spawn sh ~/.scripts/queue_video.sh {url}'
)
config.bind(
    "Z",
    "hint links spawn youtube-dl --embed-thumbnail --extract-audio --audio-format mp3 --audio-quality 0 -o ~/Music/%(title)s-%(id)s.%(ext)s {hint-url}"
)
config.bind(
    "xx",
...
config.set(
    "url.searchengines", {
        ...
        "poe": "https://pathofexile.gamepedia.com/Special:Search?search={}"
    })

# avoid askin ssl permissions
# config.set("content.ssl_strict", True)

# Remove download message after 500ms
