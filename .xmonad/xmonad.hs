...
myScratchpads = [NS "social" spawnChromium findChrom floatChrom,
    ...
    where
      ...
      floatSteam = customFloating $ W.RationalRect l t w h
             where
              h = 0.8
              w = 0.8
              t = 0.9 -h
...
myKeys conf@XConfig {XMonad.modMask = modm} =
  M.fromList $
    ...
    [ ((modm .|. shiftMask, xK_Return), spawn $ XMonad.terminal conf),
      ...
      ((modm .|. shift, xK_d), spawn "sh ~/.screenlayouts/toggle_monitors.sh"),
      ...
      -- mpv
      ((control, play), spawn $ "playerctl -p mpv play-pause"),
      ((control, next), spawn $ "playerctl -p mpv next"),
      ((control, prev), spawn $ "playerctl -p mpv previous"),
      ...
      ((modm .|. control, xK_d), spawn $ "sh " ++ scriptDir ++ "date.sh"),
      ...
      ((control, xK_Print), spawn $ "sh " ++ scriptDir ++ "screenshot.sh current"),
      ((mod4Mask, xK_Print), spawn $ "sh " ++ scriptDir ++ "screenshot.sh full"),
      ((shift, xK_Print), spawn $ "sh " ++ scriptDir ++ "screenshot.sh screen0"),
      ((shift .|. controlMask, xK_Print), spawn $ "sh " ++ scriptDir ++ "screenshot.sh screen1"),
      ((modm .|. controlMask, xK_p), spawn $ "sh " ++ scriptDir ++ "move_floating_window.sh"),
      ...
      ((shift, xK_Menu), namedScratchpadAction myScratchpads "lutris")
    ]
      ++
      --
...
-- Theme for showWName which rints current workspace when you change workspaces.
...
myManageHook = do
  composeAll (
    [ 
      className =? "thunderbird" --> doF (W.shift "NSP"),
      appName =? "chromium" --> doF (W.shift "NSP"),
      className =? "gimp" --> unfloat,
      className =? "firefox" <&&> resource =? "Toolkit" --> (customFloating $ W.RationalRect 0.5 0.3 0.4 0.4),
      className =? "rsi launcher.exe" --> unfloat,                            -- Sink the launcher so SC can be rendered on top of it
...
myActivateHook =
  className =? "Chromium" <&&> className =? "Chromium" --> doAskUrgent
...
main = do 
  ...
  xmproc0 <- spawnPipe ( "until pgrep wireplumber; do sleep 1; done && sleep 1 && xmobar -x 0 " ++ xmobarDir ++ ".xmobarrc" )
  xmproc1 <- spawnPipe ( "until pgrep wireplumber; do sleep 1; done && sleep 1 && xmobar -x 1 " ++ xmobarDir ++ ".xmobarrc" )
          ...
          dynamicLogWithPP $ filterOutWsPP [scratchpadWorkspaceTag] 
            def             
              { ppOutput = \x -> hPutStrLn xmproc0 x >> hPutStrLn xmproc1 x, -- >> hPutStrLn xmproc2 x,
                ppCurrent = xmobarColor "#D8A704" "" . wrap "<box type=Bottom width=2> " " </box>",
                ppHiddenNoWindows = xmobarColor "#666" "" . wrap " " " ",
                ppHidden = xmobarColor "#b3afc2" "" . wrap "<box type=Bottom width=1> " " </box>",
                ppTitle = xmobarColor "#ffffff" "" . shorten 0,
                ppVisible = xmobarColor "#D83C04" "" . wrap "<box type=Bottom width=1> " " </box>",
                ppUrgent = xmobarColor "red" "yellow",
                ppLayout = xmobarColor "#b3afc2" "",
                ppOrder = \(ws:lyt:_) -> [ws, lyt]
              }
      }
